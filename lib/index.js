    ...
    resolverFactory = function (configPathOrArray, basePath) {
        ...
        // configurations is initialized here and sent to resolution-provider
            ...
            load = function () {
                ...
                logger.info(startupDependencies.length, 'startup components found. Loading then now.');
                return Q.allSettled(
             ...
             * tree
             * @returns {q} a Promise that gets resolved with {@link module:base-resolver~Resolver} when the unload is
             * executed. This promise is rejected with the error if unload execution fails.
             * @memberof module:base-resolver~Resolver
             */
            unload = function () {
                logger.info('unloading components.');
                return resolver.unload().then(
                    function () {
                        logger.info('successfully unloaded all components!');
                        return resolver;
                    },
                    function (error) {
                        logger.error('error unloading components!', error);
                        throw error;
                    }
                );
            };
